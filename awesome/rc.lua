...
-- possible replacment values for default (gtk, sky, xresources, zenburn)
-- beautiful.init(gears.filesystem.get_themes_dir() .. "default/theme.lua")
beautiful.init("~/.config/awesome/theme.lua")

-- This is used later as the default terminal and editor to run.
terminal = os.getenv("TERMINAL_EMULATOR") or "alacritty"
...
-- I use nitrogen for setting wallpaper
...
awful.screen.connect_for_each_screen(function(s)
	...
	-- set blank widget, usefull for adding spacing between widgets
	...
	s.mywibox:setup({
		...
		{ -- Left widgets
			...
			-- removing awesome widget (here it's mylauncher variable)
			-- mylauncher,
			s.mytaglist,
			s.mypromptbox,
		},
...
clientkeys = gears.table.join(
	...
	awful.key({ modkey }, "e", function(c)
		c:kill()
	end, { description = "kill window", group = "client" }),
	awful.key(
		{ modkey, "Shift" },
		"f",
	...
	awful.key({ modkey }, "v", function(c)
		c.ontop = not c.ontop
	end, { description = "toggle keep on top (always keep visible)", group = "client" }),
